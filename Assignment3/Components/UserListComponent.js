...
import UserWidgetComponent from './UserWidgetComponent'; // Make sure to import the UserWidgetComponent

class UserListComponent extends Component {
  render() {
    const { users, onEdit, onDelete } = this.props; // get props

    return (
      <div style={{ padding: '20px' }}>
        <h2>User List</h2>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '20px' }}>
          {/* Iterate over the users array and display each user inside a widget */}
          {users.map(user => (
            <div key={user.id} style={{ flex: '1 1 300px', maxWidth: '300px' }}>
              <UserWidgetComponent 
                user={user} 
                onEdit={() => onEdit(user)} 
